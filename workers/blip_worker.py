...
class BlipWorker(BaseWorker):
    ...
    def process_message(self, ch, method, properties, body):
        """Process a BLIP captioning request"""
        try:
            # Parse message
            ...
            self.logger.debug(f"Processing BLIP request for image {image_id}")
            ...
            self.logger.info(f"Successfully processed BLIP captioning for image {image_id}")
            
        except Exception as e:
            self.logger.error(f"Error processing BLIP message: {e}")
            # Reject and requeue
