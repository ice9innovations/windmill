#!/usr/bin/env python3
"""
ClipWorker - clip ML service worker
"""
import sys
import os
...
class ClipWorker(BaseWorker):
    """Worker for clip ML service"""
    
    def __init__(self):
        super().__init__('clip')
    
    def process_message(self, ch, method, properties, body):
        """Process a clip ML request"""
        try:
            # Parse message
            ...
            self.logger.debug(f"Processing clip request for image {image_id}")
            ...
            self.logger.info(f"Successfully processed clip request for image {image_id}")
