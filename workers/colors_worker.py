...
class ColorsWorker(BaseWorker):
    ...
    def process_message(self, ch, method, properties, body):
        ...
        try:
            ...
            self.logger.debug(f"Processing colors request for image {image_id}")
            ...
            self.logger.info(f"Successfully processed colors request for image {image_id}")
            
        except Exception as e:
            self.logger.error(f"Error processing colors message: {e}")
            # Reject and requeue
