#!/usr/bin/env python3
"""
HaikuWorker - Claude Haiku VLM service worker

Intercepts NSFW images before calling the Claude API by checking NudeNet
results first. If explicit content is detected, a synthetic blocked result
is stored and the Claude API is never called.
"""
import sys
import os
import json
...
# NudeNet labels that indicate content Claude should not describe.
...
BLOCKED_TEXT = "[NSFW — not sent to Claude API]"

NUDENET_TIMEOUT  = 5    # seconds to wait for NudeNet result (normally ~23ms)
NUDENET_POLL     = 0.1  # seconds between polls


class HaikuWorker(BaseWorker):
    """Worker for Claude Haiku VLM service"""

    def __init__(self):
        super().__init__('primary.haiku')
    ...
    def process_message(self, ch, method, properties, body):
        ...
        # Wait for NudeNet result before calling the Claude API.
