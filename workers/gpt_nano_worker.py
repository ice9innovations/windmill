#!/usr/bin/env python3
"""
GptNanoWorker - OpenAI GPT-5 Nano VLM service worker

Intercepts NSFW images before calling the OpenAI API by checking NudeNet
results first. If explicit content is detected, a synthetic blocked result
is stored and the OpenAI API is never called.
"""
import sys
import os
import json
...
# NudeNet labels that indicate content OpenAI should not describe.
...
BLOCKED_TEXT = "[NSFW — not sent to OpenAI API]"

NUDENET_TIMEOUT  = 5    # seconds to wait for NudeNet result (normally ~23ms)
NUDENET_POLL     = 0.1  # seconds between polls


class GptNanoWorker(BaseWorker):
    """Worker for OpenAI GPT-5 Nano VLM service"""

    def __init__(self):
        super().__init__('primary.gpt_nano')
    ...
    def process_message(self, ch, method, properties, body):
        ...
        # Wait for NudeNet result before calling the OpenAI API.
        ...
        if predictions is None:
            self.logger.warning(
                f"gpt_nano: NudeNet result not available for image {image_id} "
                f"after {NUDENET_TIMEOUT}s — skipping OpenAI API call"
            )
        ...
        if detected:
            self.logger.info(
                f"gpt_nano: NSFW detected for image {image_id} "
                f"({', '.join(sorted(detected))}) — skipping OpenAI API call"
            )
    ...
    def _fetch_nudenet(self, image_id):
        ...
        try:
            ...
            self.logger.error(f"gpt_nano: nudenet fetch error for image {image_id}: {e}")
            return None
    ...
    def _store_blocked(self, image_id, reason):
        ...
        try:
            ...
            cursor.execute(
                ...
                (image_id, 'gpt_nano', json.dumps(result), 'success', self.worker_id)
            )
            self.db_conn.commit()
            cursor.close()
        except Exception as e:
            self.logger.error(
                f"gpt_nano: failed to store blocked result for image {image_id}: {e}"
            )
