...
class XceptionWorker(BaseWorker):
    ...
    def process_message(self, ch, method, properties, body):
        ...
        try:
            ...
            self.logger.debug(f"Processing xception request for image {image_id}")
            
            # Call xception service
            ...
            self.logger.info(f"Successfully processed xception request for image {image_id}")
            
        except Exception as e:
            self.logger.error(f"Error processing xception message: {e}")
            # Reject and requeue
