#!/usr/bin/env python3
"""
GeminiWorker - Google Gemini VLM service worker

Intercepts NSFW images before calling the Gemini API by checking NudeNet
results first. If explicit content is detected, a synthetic blocked result
is stored and the Gemini API is never called.
"""
import sys
import os
import json
...
# NudeNet labels that indicate content Gemini should not describe.
...
BLOCKED_TEXT = "[NSFW — not sent to Gemini API]"
...
class GeminiWorker(BaseWorker):
    """Worker for Google Gemini VLM service"""

    def __init__(self):
        super().__init__('primary.gemini')
    ...
    def process_message(self, ch, method, properties, body):
        ...
        # Wait for NudeNet result before calling the Gemini API.
        ...
        if predictions is None:
            self.logger.warning(
                f"gemini: NudeNet result not available for image {image_id} "
                f"after {NUDENET_TIMEOUT}s — skipping Gemini API call"
            )
        ...
        if detected:
            self.logger.info(
                f"gemini: NSFW detected for image {image_id} "
                f"({', '.join(sorted(detected))}) — skipping Gemini API call"
            )
    ...
    def _fetch_nudenet(self, image_id):
        ...
        try:
            ...
            self.logger.error(f"gemini: nudenet fetch error for image {image_id}: {e}")
            return None
    ...
    def _store_blocked(self, image_id, reason):
        ...
        try:
            ...
            cursor.execute(
                ...
                (image_id, 'gemini', json.dumps(result), 'success', self.worker_id)
            )
            self.db_conn.commit()
            cursor.close()
        except Exception as e:
            self.logger.error(
                f"gemini: failed to store blocked result for image {image_id}: {e}"
            )
